...
const URL_KEYS = ["url", "endpoint", "baseUrl", "base_url", "server", "host", "api_url"] as const;
...
export class PR07TransportEvaluator implements ControlEvaluator {
  ...
  evaluate(action: Action, _config: ResolvedConfig): ControlResult {
    ...
    const candidateUrls = extractUrls(action.parameters.raw as Record<string, unknown>);
