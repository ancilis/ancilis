...
def test_check_posture_returns_latest_session_active_evaluator_results() -> None:
    ...
    assert structured["posture_score"] == 0.5
    assert structured["active_overlays"] == ["glba", "soc2"]
    assert structured["evaluated_at"]
    assert [control["control_id"] for control in structured["controls"]] == ["PR-01", "PR-02"]
...
def test_list_overlays_reports_active_overlay_coverage() -> None:
    ...
    _content, structured = asyncio.run(server.call_tool("ancilis_list_overlays", {}))

    overlays = {overlay["overlay_id"]: overlay for overlay in structured["overlays"]}
    ...
    for overlay_id in ("glba", "soc2"):
        ...
        assert overlays[overlay_id]["name"] == overlay_profiles[overlay_id]["name"]
        assert overlays[overlay_id]["controls_with_evidence"] == expected_covered
        assert overlays[overlay_id]["total_controls"] == len(expected_controls)
        assert overlays[overlay_id]["coverage_percent"] == expected_percent
...
def test_list_overlays_handles_no_evidence() -> None:
    ...
    assert {
        (overlay["overlay_id"], overlay["coverage_percent"], overlay["controls_with_evidence"])
